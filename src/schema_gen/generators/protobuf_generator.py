...
class ProtobufGenerator:
    ...
    def generate_file(self, schema: USRSchema) -> str:
        ...
        base_message = self._generate_single_message(schema.name, schema.description, base_fields, is_base_message=True)
        ...
        for variant_name in schema.variants.keys():
            ...
            variant_message_name = self._variant_to_message_name(schema.name, variant_name)
            variant_message = self._generate_single_message(variant_message_name, schema.description, variant_fields)
    ...
    def _generate_single_message(self, message_name: str, description: str, 
                                fields: List[USRField], is_base_message: bool = False) -> str:
        ...
        for field in fields:
            if field.type == FieldType.LITERAL and field.literal_values:
                ...
                for i, value in enumerate(field.literal_values):
                    enum_value_name = str(value).upper().replace(' ', '_').replace('-', '_')
    ...
    def _generate_service_definition(self, schema: USRSchema) -> Optional[str]:
        """Generate gRPC service definition if appropriate"""
        # Only generate service for schemas that have CRUD-like variants
        crud_variants = []
        for variant_name in schema.variants.keys():
            if any(crud in variant_name.lower() for crud in ['create', 'update', 'get', 'list', 'delete']):
        ...
        if any('create' in v.lower() for v in crud_variants):
            create_variant = next((v for v in crud_variants if 'create' in v.lower()), None)
            if create_variant:
                create_msg = self._variant_to_message_name(schema.name, create_variant)
                lines.append(f'  rpc Create{schema.name}({create_msg}) returns ({schema.name});')
        
        lines.append(f'  rpc Get{schema.name}(Get{schema.name}Request) returns ({schema.name});')
        lines.append(f'  rpc List{schema.name}(List{schema.name}Request) returns (List{schema.name}Response);')
        
        if any('update' in v.lower() for v in crud_variants):
            update_variant = next((v for v in crud_variants if 'update' in v.lower()), None)
            if update_variant:
                update_msg = self._variant_to_message_name(schema.name, update_variant)
                lines.append(f'  rpc Update{schema.name}({update_msg}) returns ({schema.name});')
        
        lines.append(f'  rpc Delete{schema.name}(Delete{schema.name}Request) returns (google.protobuf.Empty);')
        lines.append('}')
        ...
        # Find ID field
        id_field = next((f for f in schema.fields if f.primary_key or f.name.lower() == 'id'), None)
        if id_field:

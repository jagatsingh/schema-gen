...
class JacksonGenerator:
    ...
    def generate_file(self, schema: USRSchema) -> str:
        ...
        # Generate base class
        base_class = self._generate_single_class(schema.name, schema.description, schema.fields, is_base_class=True)
        ...
        for variant_name in schema.variants.keys():
            ...
            variant_class = self._generate_single_class(variant_class_name, schema.description, variant_fields)
            lines.append(variant_class)
            lines.append('')
        
        return '\n'.join(lines).rstrip() + '\n'
    
    def _generate_single_class(self, class_name: str, description: str, 
                              fields: List[USRField], is_base_class: bool = False) -> str:
        ...
        lines.extend([
            ...
            '@JsonPropertyOrder({' + ', '.join(f'"{f.name}"' for f in fields) + '})',
        ...
        if fields:
            ...
            for field in fields:
                ...
                constructor_assignments.append(f'        this.{field.name} = {field.name};')
    ...
    def _generate_field_definition(self, field: USRField) -> List[str]:
        ...
        if field.format_type == 'date-time':
            annotations.append('    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd\'T\'HH:mm:ss.SSSZ")')
        elif field.format_type == 'date':
            annotations.append('    @JsonFormat(shape = JsonFormat.Shape.STRING, pattern = "yyyy-MM-dd")')
    ...
    def _get_required_imports(self, schema: USRSchema) -> List[str]:
        ...
        has_date_fields = any(f.type in [FieldType.DATE, FieldType.DATETIME] for f in schema.fields)
        if has_date_fields:
        ...
        has_string_validation = any(f.type == FieldType.STRING and (f.min_length or f.max_length) 
        ...
        has_numeric_validation = any(f.type in [FieldType.INTEGER, FieldType.FLOAT] and 
                                    (f.min_value or f.max_value) for f in schema.fields)

...
class GraphQLGenerator:
    ...
    def generate_file(self, schema: USRSchema) -> str:
        ...
        base_type = self._generate_single_type(schema.name, schema.description, base_fields, is_base_type=True)
        ...
        for variant_name in schema.variants.keys():
            ...
            variant_type = self._generate_single_type(variant_type_name, schema.description, variant_fields)
    ...
    def _generate_single_type(self, type_name: str, description: str, 
                             fields: List[USRField], is_base_type: bool = False) -> str:
    ...
    def _generate_input_types(self, schema: USRSchema) -> List[str]:
        ...
        create_fields = [f for f in schema.fields if not f.primary_key and not f.auto_increment]
        if create_fields:
            input_type = self._generate_input_type(f"{schema.name}Input", "Input for creating/updating", create_fields)
        ...
        for variant_name in schema.variants.keys():
            if 'create' in variant_name.lower() or 'input' in variant_name.lower():
                variant_fields = schema.get_variant_fields(variant_name)
                variant_input_name = self._variant_to_type_name(schema.name, variant_name) + "Input"
                variant_input = self._generate_input_type(variant_input_name, f"Input for {variant_name}", variant_fields)
                input_types.append(variant_input)
        
        return input_types
    
    def _generate_input_type(self, input_name: str, description: str, fields: List[USRField]) -> str:

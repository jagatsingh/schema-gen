...
class SqlAlchemyGenerator:
    ...
    def generate_file(self, schema: USRSchema) -> str:
        ...
        """
        all_imports = set(['sqlalchemy', 'sqlalchemy.orm'])
        all_columns = []
        all_relationships = []
    ...
    def _generate_relationship_definition(self, field: USRField) -> tuple[str, set[str]]:
    ...
    def _generate_complete_file(self, schema_name: str, imports: set, 
                              columns: List[str], relationships: List[str],
                              description: str) -> str:
        ...
        # Add imports
        lines.append("from sqlalchemy import Column, Integer, String, Boolean, DateTime, Float")
        ...
        for imp in sorted(imports):
            if imp.startswith('sqlalchemy.') and imp not in ['sqlalchemy', 'sqlalchemy.orm']:

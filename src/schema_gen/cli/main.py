...
class SchemaWatcher(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if not (event.src_path.endswith('.py') or event.src_path.endswith(self.config_path)):
            return
...
@main.command()
@click.option('--input', '-i', 'input_dir', help='Input directory containing schemas', default=None)
@click.option('--output', '-o', 'output_dir', help='Output directory for generated files', default=None)
@click.option('--target', '-t', 'targets', multiple=True, help='Target generators to run', default=None)
@click.option('--config', '-c', 'config_path', help='Path to config file', default='.schema-gen.config.py')
...
def validate(config_path):
    ...
    try:
        ...
        if validation_passed:
            ...
            click.echo("❌ Some schemas are out of date. Run 'schema-gen generate' to update.")
            raise click.Abort()
...
@click.option('--output-dir', help='Output directory for generated files', default='generated/')
...
@main.command()
@click.option('--install-pre-commit/--no-install-pre-commit', default=True, help='Install pre-commit package')
...
"""
    ...
    if install_pre_commit:
        try:
            ...
            click.echo("💡 Install manually with: pip install pre-commit && pre-commit install")
